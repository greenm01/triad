...
suite "Layout Algorithm Math":
  ...
  test "layoutMasterStack splits screen accurately":
    ...
    tag.columns.add(ProjectedColumn(windows: @[102], widthProportion: 1.0))
  ...
  test "deck layout keeps stack windows in one deck area":
    ...
    tag.columns.add(
      ProjectedColumn(
        windows: @[ProjectionWindowId(101), 102, 103], widthProportion: 1.0
      )
    )
  ...
  test "vertical grid fills rows before columns":
    var tag = ProjectedTag(tagId: 1, layoutMode: LayoutMode.VerticalGrid)
    tag.columns.add(
      ProjectedColumn(
        windows: @[ProjectionWindowId(101), 102, 103, 104], widthProportion: 1.0
      )
    )
