...
proc activeWorkspace(
    snapshot: ShellSnapshot): tuple[found: bool; workspace: ShellWorkspace] =
...
proc focusedShellWindow(
    snapshot: ShellSnapshot): tuple[count: int; win: ShellWindow] =
  for win in snapshot.windows:
    if win.isFocused:
      inc result.count
      result.win = win
...
proc requireNiriProjection(context: string; snapshot: ShellSnapshot) =
  let workspacesReply =
    niri_compat.handleNiriRequest("\"Workspaces\"", snapshot)
  ...
  require(context, workspaces.len == snapshot.workspaces.len,
  ...
  if snapshot.workspaces.len > 0:
    require(context, activeCount == 1,
      "Niri projection must expose one active workspace")
  ...
  require(context, windows.len == snapshot.windows.len,
  ...
  require(context, focusedCount <= 1,
    "Niri projection must expose at most one focused window")

  let focusedReply =
    niri_compat.handleNiriRequest("\"FocusedWindow\"", snapshot)
...
proc requireTagShellSemantics*(model: Model; context = "tag semantics") =
  ...
  require(context, focused.count <= 1,
    "snapshot must expose at most one globally focused window")

  if snapshot.workspaces.len > 0:
    require(context, active.found,
      "snapshot with workspaces must expose an active workspace")
    require(context,
      active.workspace.workspaceIdx == snapshot.activeWorkspaceIdx,
      "active workspace index mismatch")
    require(context, active.workspace.tagId == snapshot.activeTag,
  ...
  if active.found and active.workspace.focusedWindow != WindowId(0):
    ...
    if activeWin.found and not activeWin.win.isMinimized:
      ...
      require(context, focused.win.id == active.workspace.focusedWindow,
        "global focused window must match active workspace focus")

  if focused.count == 1:
    require(context, active.found,
      "focused shell window requires an active workspace")
    require(context, focused.win.workspaceIdx == active.workspace.workspaceIdx,
      "focused shell window must be on active workspace index")
    require(context, focused.win.tagId.isSome,
      "focused shell window must have a tag")
    require(context, focused.win.tagId.get() == active.workspace.tagId,
      "focused shell window must be on active workspace tag")

  for win in snapshot.windows:
    if win.tagId.isSome:
      let workspace = snapshot.workspaceByTag(win.tagId.get())
      require(context, workspace.found,
        "tagged shell window has no visible workspace")
      require(context, workspace.workspace.workspaceIdx == win.workspaceIdx,
    ...
    if win.workspaceIdx != 0:
      ...
      if win.tagId.isSome:
        require(context, workspace.workspace.tagId == win.tagId.get(),
          "shell window tag does not match indexed workspace")
    if win.isFocused:
      require(context, active.found,
        "focused window requires active workspace")
      require(context, win.workspaceIdx == active.workspace.workspaceIdx,
