...
const CommonTerminalCommands* = [
  "kgx", "gnome-terminal", "konsole", "xfce4-terminal", "mate-terminal", "lxterminal",
  "foot", "kitty", "alacritty", "wezterm", "ghostty", "xterm",
]
...
proc terminalCandidates*(
    configuredCommand: seq[string], envTerminal = getEnv("TERMINAL", "")
): seq[seq[string]] =
...
proc resolveTerminalCommand*(
    configured = getEnv("TERMINAL", ""),
    exists: proc(command: string): bool {.closure.} = commandExists,
): seq[string] =
