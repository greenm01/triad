...
proc intFromField(node: JsonNode; field: string): Option[int] =
  if node.kind != JObject or not node.hasKey(field): return none(int)
  try:
    if node[field].kind == JInt: return some(node[field].getInt())
  except: discard
  none(int)

proc floatFromField(node: JsonNode; field: string): Option[float] =
  if node.kind != JObject or not node.hasKey(field): return none(float)
  try:
    if node[field].kind == JFloat: return some(node[field].getFloat())
    if node[field].kind == JInt: return some(float(node[field].getInt()))
  except: discard
  none(float)

proc actionToMsg(action: string; payload: JsonNode; snapshot: ShellSnapshot): Option[Msg] =
  case action
  ...
  of "focus-occupied-tag-right": some(Msg(kind: MsgKind.CmdFocusOccupiedTagRight))
  ...
  of "fullscreen-window": some(Msg(kind: MsgKind.CmdToggleFullscreen))
  of "toggle-fullscreen": some(Msg(kind: MsgKind.CmdToggleFullscreen))
  of "maximize-window-to-edges": some(Msg(kind: MsgKind.CmdToggleMaximized))
  of "toggle-maximized": some(Msg(kind: MsgKind.CmdToggleMaximized))
  ...
  of "move-window-up-or-to-workspace-up": some(Msg(kind: MsgKind.CmdMoveWindowUpOrToWorkspaceUp))
  of "move-window-down-or-to-workspace-down": some(Msg(kind: MsgKind.CmdMoveWindowDownOrToWorkspaceDown))
  of "focus-tag":
    let t = uintFromField(payload, "tag")
    if t.isSome: some(Msg(kind: MsgKind.CmdFocusTag, focusTag: t.get())) else: none(Msg)
  of "move-to-tag":
    let t = uintFromField(payload, "tag")
    if t.isSome: some(Msg(kind: MsgKind.CmdMoveToTag, targetTag: t.get())) else: none(Msg)
  of "focus-workspace":
    let i = uintFromField(payload, "workspace_idx")
    if i.isSome: some(Msg(kind: MsgKind.CmdFocusWorkspaceIndex, workspaceIndex: i.get())) else: none(Msg)
  of "move-to-workspace":
    let i = uintFromField(payload, "workspace_idx")
    if i.isSome: some(Msg(kind: MsgKind.CmdMoveToWorkspaceIndex, workspaceIndex: i.get())) else: none(Msg)
  of "focus-window":
    let id = uintFromField(payload, "id")
    if id.isSome: some(Msg(kind: MsgKind.CmdFocusWindowById, focusWindowId: WindowId(id.get()))) else: none(Msg)
  of "close-window":
    let id = uintFromField(payload, "id")
    if id.isSome: some(Msg(kind: MsgKind.CmdCloseWindowById, closeWindowId: WindowId(id.get())))
    else: some(Msg(kind: MsgKind.CmdCloseWindow))
  of "rename-tag":
    let name = stringFromField(payload, "name")
    if name.len > 0: some(Msg(kind: MsgKind.CmdRenameTag, newName: name)) else: none(Msg)
  of "toggle-named-scratchpad":
    let name = stringFromField(payload, "name")
    if name.len > 0: some(Msg(kind: MsgKind.CmdToggleNamedScratchpad, scratchpadName: name)) else: none(Msg)
  of "move-to-named-scratchpad":
    let name = stringFromField(payload, "name")
    if name.len > 0: some(Msg(kind: MsgKind.CmdMoveToNamedScratchpad, scratchpadName: name)) else: none(Msg)
  of "resize-width":
    let d = floatFromField(payload, "delta")
    if d.isSome: some(Msg(kind: MsgKind.CmdResizeWidth, deltaW: d.get())) else: none(Msg)
  of "resize-height":
    let d = floatFromField(payload, "delta")
    if d.isSome: some(Msg(kind: MsgKind.CmdResizeHeight, deltaH: d.get())) else: none(Msg)
  of "adjust-master-ratio":
    let d = floatFromField(payload, "delta")
    if d.isSome: some(Msg(kind: MsgKind.CmdAdjustMasterRatio, deltaMR: d.get())) else: none(Msg)
  of "master-ratio":
    let v = floatFromField(payload, "value")
    if v.isSome: some(Msg(kind: MsgKind.CmdSetMasterRatio, ratio: v.get())) else: none(Msg)
  of "adjust-gaps":
    let d = floatFromField(payload, "delta")
    if d.isSome: some(Msg(kind: MsgKind.CmdAdjustGaps, deltaG: int32(d.get()))) else: none(Msg)
  of "master-count":
    let c = intFromField(payload, "count")
    if c.isSome: some(Msg(kind: MsgKind.CmdSetMasterCount, count: c.get())) else: none(Msg)
  of "adjust-master-count":
    let d = intFromField(payload, "delta")
    if d.isSome: some(Msg(kind: MsgKind.CmdAdjustMasterCount, deltaMC: d.get())) else: none(Msg)
  of "move-floating":
    let dx = floatFromField(payload, "dx"); let dy = floatFromField(payload, "dy")
    if dx.isSome and dy.isSome: some(Msg(kind: MsgKind.CmdMoveFloating, moveDX: int32(dx.get()), moveDY: int32(dy.get()))) else: none(Msg)
  of "resize-floating":
    let dw = floatFromField(payload, "dw"); let dh = floatFromField(payload, "dh")
    if dw.isSome and dh.isSome: some(Msg(kind: MsgKind.CmdResizeFloating, deltaFW: int32(dw.get()), deltaFH: int32(dh.get()))) else: none(Msg)
  else:
    none(Msg)
