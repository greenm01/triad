...
proc drawChar(
    buf: var PixelBuffer, x, y: int32, ch: char, color: uint32, scale = HotkeyFontScale
) =
...
proc renderHotkeyOverlayBuffer*(
    ...
    if rows.len == 0:
      ...
      min(
        ...
        max(
          1,
          int(
            (max(240'i32, screen.h - 120'i32) - HotkeyPadding * 3 - rowH) div
              (rowH + HotkeyRowGap)
          ),
        ),
      )
  let height =
    HotkeyPadding * 3 + rowH + int32(visibleRows) * rowH +
  ...
  for row in rowsToDraw:
    ...
    result.drawText(labelX, y, width - labelX - HotkeyPadding, row.label, HotkeyMuted)
    y += rowH + HotkeyRowGap
