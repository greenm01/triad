...
const
  RiverEdgeTop = 1'u32
  RiverEdgeLeft = 4'u32
  RiverAllEdges = RiverEdgeTop or RiverEdgeBottom or
    RiverEdgeLeft or RiverEdgeRight
  RiverDecorationOnlySupportsCsd = 0'u32
  AllWatchedModifiers = 1'u32 or 4'u32 or 8'u32 or 32'u32 or
    64'u32 or 128'u32
...
proc bindingModeActive(daemon: TriadDaemon; mode: BindingMode): bool =
  ...
  of BindingMode.BindNormal: not daemon.currentModel.overviewActive
  of BindingMode.BindOverview: daemon.currentModel.overviewActive

proc keyBindingActive(
    daemon: TriadDaemon; binding: KeyBindingConfig): bool =
...
proc pointerBindingActive(
    daemon: TriadDaemon; binding: PointerBindingConfig): bool =
  if not daemon.bindingModeActive(binding.mode):
    return false
  if daemon.currentModel.keyboardShortcutsInhibited() and
      not binding.bypassShortcutsInhibit:
...
proc onSeatShellSurfaceInteraction(
    data: pointer; seat: ptr RiverSeatV1;
...
proc onXkbStopRepeat(data: pointer; binding: ptr riverXkb.RiverXkbBindingV1) =
  ...
  trace "XKB binding stop-repeat", bindingId = id,
    count = daemon.xkbStopRepeatCount[id]

var xkbBindingListener* = riverXkb.RiverXkbBindingV1Listener(
  pressed: onXkbPressed,
  released: onXkbReleased,
  stopRepeat: onXkbStopRepeat
)

proc onXkbSeatAteUnboundKey(
    data: pointer; seat: ptr riverXkb.RiverXkbBindingsSeatV1) =
  ...
  trace "XKB seat ate unbound key", xkbSeatId = id,
    count = daemon.xkbSeatAteUnbound[id]

proc onXkbSeatModifiersUpdate(
    data: pointer; seat: ptr riverXkb.RiverXkbBindingsSeatV1; old: uint32;
    new: uint32) =
...
var xkbSeatListener* = riverXkb.RiverXkbBindingsSeatV1Listener(
  ateUnboundKey: onXkbSeatAteUnboundKey,
  modifiersUpdate: onXkbSeatModifiersUpdate
)

proc onPointerBindingPressed(
...
var pointerBindingListener* = RiverPointerBindingV1Listener(
  pressed: onPointerBindingPressed,
  released: onPointerBindingReleased
)
...
proc attachLayerOutput*(daemon: var TriadDaemon; outputId: uint32) =
  if daemon.riverLayerShell == nil or
      not daemon.outputPointers.hasKey(outputId) or
      daemon.layerOutputPointers.hasKey(outputId):
    return
  let layerOutput = daemon.riverLayerShell.getOutput(
    daemon.outputPointers[outputId])
...
proc addXkbBinding(
    daemon: var TriadDaemon; seat: ptr RiverSeatV1;
    bindingConfig: KeyBindingConfig; keysym, modifiers: uint32; msg: Msg) =
...
proc addPointerBinding(
    daemon: var TriadDaemon; seat: ptr RiverSeatV1;
    bindingConfig: PointerBindingConfig) =
  var msg = none(Msg)
  ...
  for seat in daemon.seatPointers:
    ...
    for binding in daemon.currentModel.keyBindings:
      ...
      if parsed.isSome and sym != 0:
        daemon.addXkbBinding(seat, binding, sym, binding.modifiers,
          parsed.get())

    for binding in daemon.currentModel.pointerBindings:
      if daemon.pointerBindingActive(binding):
        daemon.addPointerBinding(seat, binding)
    if daemon.currentModel.overviewActive and
        not daemon.hasOverviewLeftClickBinding():
...
proc applyManageState*(daemon: var TriadDaemon) =
  ...
  for id, win in daemon.windowPointers.pairs:
    ...
    if dataOpt.isSome:
      ...
      if data.hasDecorationHint and
          data.decorationHint == RiverDecorationOnlySupportsCsd:
        win.useCsd()
      else:
        win.useSsd()
  ...
  for seat in daemon.seatPointers:
    if daemon.currentModel.cursor.theme.len > 0:
      ...
      seat.setXcursorTheme(cstring(daemon.currentModel.cursor.theme),
        cursorSize)
    if daemon.currentModel.layerFocusExclusive or
        daemon.currentModel.sessionLocked:
      seat.clearFocus()
    elif daemon.currentModel.overviewActive and
        daemon.ownedShellSurfaceId != 0 and
        daemon.shellSurfacePointers.hasKey(daemon.ownedShellSurfaceId):
      seat.focusShellSurface(daemon.shellSurfacePointers[
        daemon.ownedShellSurfaceId])
