...
import ../core/msg
import bindings_runtime, manage_requests, message_queue,
...
proc handleGlobal*(
  ...
  if interfaceName == "river_window_manager_v1":
    if version < 4'u32:
      ...
      quit 1
    daemon.riverManager = cast[ptr RiverWindowManagerV1](registry.`bind`(name,
        river_window_manager_v1_interface.addr, 4'u32))
    discard daemon.riverManager.addListener(
      riverManagerListener.addr,
      daemonData(daemon[]))
    ...
    daemon.compositor = cast[ptr Compositor](registry.`bind`(name,
    ...
    daemon.shm = cast[ptr Shm](registry.`bind`(name,
        wlCore.wl_shm_interface.addr, min(version, 1'u32)))
    ...
    let wlOutput = cast[ptr Output](registry.`bind`(name,
        wlCore.wl_output_interface.addr, min(version, 4'u32)))
    ...
    discard wlOutput.addListener(
      wlOutputListener.addr,
      cast[pointer](daemon.wlOutputListenerData[name]))
    ...
    daemon.riverLayerShell = cast[ptr riverLayer.RiverLayerShellV1](
      registry.`bind`(
        name,
        riverLayer.river_layer_shell_v1_interface.addr,
        min(version, 1'u32)))
    ...
    daemon.riverXkbBindings = cast[ptr riverXkb.RiverXkbBindingsV1](
      registry.`bind`(
        name,
        riverXkb.river_xkb_bindings_v1_interface.addr,
        min(version, 3'u32)))
    ...
    daemon.singlePixelManager =
      cast[ptr singlepixel.WpSinglePixelBufferManagerV1](
        registry.`bind`(
          name,
...
proc handleGlobalRemove*(
    data: pointer; registry: ptr Registry; name: uint32) =
  let daemon = daemonFromData(data)
  if daemon == nil:
...
var registryListener* = RegistryListener(
  global: handleGlobal,
  globalRemove: handleGlobalRemove
)
