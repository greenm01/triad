...
| Output | Monitor power | `disable_monitor`, `enable_monitor`, `toggle_monitor` | External output management | `output "name" { enabled; disabled }`, `power-off-monitors`, `power-on-monitors` | X | Declarative config can enable or disable outputs. Runtime monitor power commands disable the currently enabled outputs and restore that saved set without mutating output config rules. |
