...
| Window rules | Visual effects | `noblur`, `isnoborder`, opacity, animation flags | WM/render policy | `border`, `enable-animations`, `animation-speed` | | Rule-level border width and colors are supported; opacity, blur, shadows, radius, and per-window animation policy are not. |
