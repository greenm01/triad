...
| Window rules | Visual effects | `noblur`, `isnoborder`, opacity, animation flags | WM/render policy | `border`, `focus-ring`, `enable-animations`, `animation-speed` | | Rule-level border and focused-only focus-ring width/colors are supported; opacity, blur, shadows, radius, and per-window animation policy are not. |
