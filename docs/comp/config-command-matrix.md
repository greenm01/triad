...
| Done | Output rules | `output "name" { focus-at-startup; workspaces ...; mode; scale; position; transform; adaptive-sync }` | Niri `output`; Mango `monitorrule` | Implemented for startup focus, pinned workspace/output affinity, and wlroots output-management mode/scale/position/transform/adaptive-sync config. Strict validation rejects unknown, malformed, or unsupported output fields before startup/reload. | Output disabling, mirroring, auto placement, reserved areas, color/HDR/ICC fields, and custom modes remain intentionally unexposed. |
...
| Output | Monitor rules | `monitorrule` | `zwlr_output_manager_v1` | `output "name" { mode; scale; position; transform; adaptive-sync }` | X | Triad applies advertised output modes and related output-management fields at startup and config reload; unsupported Hyprland-style fields are strict validation errors. Output disabling and custom modes are not exposed. |
