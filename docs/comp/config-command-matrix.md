...
| Done | Output rules | `output { monitor "name" { focus-at-startup; workspaces ...; mode; scale; position; transform; adaptive-sync; vrr; enabled; disabled; reserved_area }; default { ... } }` | Niri `output`; Mango/Hyprland `monitorrule` | Implemented for grouped and legacy output rules, startup focus, pinned workspace/output affinity, and wlroots output-management mode/scale/position/transform/adaptive-sync/enablement config, plus additive Triad usable-area reservations. Strict validation rejects unknown, malformed, or River-blocked output fields before startup/reload. | Mirroring, bit depth, color/HDR/ICC fields, and compositor-specific luminance metadata remain future River/protocol features. |
