...
| IPC | Native state queries | | Shell/WM IPC | `triad msg state`, `workspaces`, `outputs`, `windows`, `focused-window`, `overview-state`, `keyboard-layouts` | X | Returns native Triad shell snapshot projections without going through the temporary Niri compatibility bridge. |
