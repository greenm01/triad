...
| Tags | Create workspace | | WM policy | `new-workspace` | X | Reuses an inactive empty dynamic workspace on the currently focused output when available; otherwise creates the next dynamic workspace and focuses it. Empty dynamic workspaces are pruned after leaving them. |
