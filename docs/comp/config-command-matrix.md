...
| Output | Monitor rules | `monitorrule` | `zwlr_output_manager_v1` | `output "name" { mode; scale; position; transform; adaptive-sync }` | X | Triad applies advertised output modes and related output-management fields at startup and config reload; output disabling and custom modes are not exposed. |
